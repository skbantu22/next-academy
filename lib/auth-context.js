...
// Only the Admin SDK can safely mint a STU-### (the shared counter is
// denied to client writes by Firestore rules), so a self-registered
// student's human-readable ID is assigned here via a self-scoped server
// endpoint right after their profile exists. Best-effort: a failure here
// must never block sign-in — the same call retries on the student's next
// login until it succeeds.
async function ensureStudentId(user) {
  try {
    const token = await user.getIdToken();
    const response = await fetch("/api/auth/ensure-student-id", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await response.json();
    return response.ok ? data.studentId : null;
  } catch {
    return null;
  }
}
...
async function createProfile(user, role = "Student") {
  ...
  if (!existing.exists()) {
    ...
    const studentId = role === "Student" ? await ensureStudentId(user) : null;
    return { uid: user.uid, role, status, ...(studentId ? { studentId } : {}) };
  }
  const data = existing.data();
  if (data.role === "Student" && !data.studentId) {
    const studentId = await ensureStudentId(user);
    if (studentId) return { uid: user.uid, ...data, studentId };
  }
