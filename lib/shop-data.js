...
// Admin/Director reassigning (or clearing) an order's real pickup/
// collection session after the fact — same direct-write permission as
// updateOrderStatus above (firestore.rules' `orders` update rule is
// admin()-only, no field restrictions). `session` is a real classSessions
// doc (or null to clear it) — never a fabricated date.
export async function updateOrderCollection(orderId, session) {
  if (!db) throw new Error("This feature isn't available right now.");
  await updateDoc(doc(db, "orders", orderId), {
    collection: session
      ? { sessionId: session.id, title: session.title || "", date: session.date || "", startTime: session.startTime || "", endTime: session.endTime || "", location: session.location || "" }
      : null,
