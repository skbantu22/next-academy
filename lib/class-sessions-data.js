...
// Every real class session — Admin/Director only (firestore.rules'
// `admin()` branch), used to let them assign/change an order's real
// pickup/collection session from the actual set that exists, rather than
// only the buyer's own enrolled ones.
export function subscribeAllClassSessions(onData, onError) {
  if (!db) return () => {};
  return onSnapshot(
