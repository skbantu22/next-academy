...
const EMAILS = ["ve.newstudent@example.com", "ve.oldunverified@example.com", "ve.verified@example.com"];
...
// A pre-existing UNVERIFIED account (simulates someone who registered
// before this change and never verified).
let u;
try { u = await auth.getUserByEmail("ve.oldunverified@example.com"); } catch { u = await auth.createUser({ email: "ve.oldunverified@example.com", password: "Test123456!", emailVerified: false, displayName: "VE Old Unverified" }); }
...
// A pre-existing VERIFIED account (must NOT be forced through verify-email).
let v;
try { v = await auth.getUserByEmail("ve.verified@example.com"); } catch { v = await auth.createUser({ email: "ve.verified@example.com", password: "Test123456!", emailVerified: true, displayName: "VE Verified" }); }
